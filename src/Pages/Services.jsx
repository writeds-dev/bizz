...
>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 max-w-7xl mx-auto">
    {servicesData.map(({ category, description, tags }, idx) => (
      <motion.div
        key={category}
        className="group relative border border-gray-700 rounded-3xl p-6 sm:p-8 cursor-pointer hover:shadow-2xl hover:shadow-lime-500 transition-all duration-300 flex flex-col min-w-0"
        variants={fadeUpDelayVariant(idx * 0.15)}
      >
        {/* Icon accent */}
        <div className="absolute -top-6 left-6 sm:left-8 w-12 h-12 sm:w-14 sm:h-14 bg-lime-400 rounded-full flex items-center justify-center shadow-lg shadow-lime-600/50 text-black font-bold text-xl sm:text-2xl">
          {category.charAt(0)}
        </div>

        {/* Category */}
        <h3 className="text-white text-xl sm:text-2xl font-semibold mb-4 sm:mb-5 pl-14 sm:pl-16 tracking-wide truncate">
          {category}
        </h3>
...
>
  ...
  <h2 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight leading-tight sm:leading-snug md:leading-tight">
    CREATIVE{" "}
    <span className="italic ml-40 bg-lime-500 ">
      Process
    </span>
  </h2>
  {/* Optional Subheading */}
  <p className="text-lg sm:text-xl text-gray-400 mt-4 md:mt-6 max-w-xl mx-auto md:mx-0">
    Explore how our innovative and collaborative process drives exceptional results, making us leaders in design and creativity.
  </p>
</div>


    <p className="max-w-full md:max-w-lg text-gray-400 leading-relaxed text-center md:text-left text-base sm:text-lg mt-6 md:mt-0">
      Let's break the mold. We're not afraid to challenge the status quo. Our team thrives on creating unique and memorable designs that make a statement.
    </p>
  </motion.div>
...
>
  {/* Left Text */}
  <div className="md:w-1/2 max-w-full mb-12 md:mb-0 text-center md:text-left">
    <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold leading-tight tracking-tight">
      Our <br />
      <span className="text-white">
        Multidisciplinary team brings together diverse talents allowing
      </span>
      <br />
      <span className="text-gray-600 text-base sm:text-lg md:text-base mt-4 block">
        We approach projects with a holistic perspective that fuses creativity and functionality.
      </span>
    </h1>

    {/* Scroll Down */}
    <div className="mt-12 sm:mt-16 flex items-center gap-3 cursor-pointer group max-w-max mx-auto md:mx-0">
  ...
  {/* Right Image */}
  <div className="md:w-1/2 flex justify-center md:justify-end w-full max-w-[480px] mx-auto">
    <img
      src="https://res.cloudinary.com/dcqfpnls6/image/upload/v1748761234/3d-rendering-abstract-black-white-background_ulelcn.jpg"
      alt="Team working"
      className="rounded-xl max-w-full w-full object-cover shadow-xl transition-transform duration-500 hover:scale-105"
    />
  </div>
</motion.section>
...
>
  ...
  <div
    className="absolute top-10 right-6 md:right-20 text-yellow-400 text-3xl sm:text-4xl cursor-pointer select-none animate-pulse hover:text-lime-500 transition-colors"
